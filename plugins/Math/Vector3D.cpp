...
{
    return Math::Vector3D(random_double(min, max), random_double(min, max),
                          random_double(min, max));
}

Math::Vector3D cross(const Math::Vector3D &u, const Math::Vector3D &v)
{
    return Math::Vector3D(u.y * v.z - u.z * v.y, u.z * v.x - u.x * v.z, u.x * v.y - u.y * v.x);
}
...
Math::Vector3D random_unit_vector() { return unit_vector(random_in_unit_sphere()); }
