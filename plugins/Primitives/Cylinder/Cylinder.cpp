...
RayTracer::Cylinder::Cylinder(const Math::Point3D &center, double radius, const char axe)
{
    this->center = center;
    this->radius = radius;
...
bool RayTracer::Cylinder::hits(const RayTracer::Ray &ray, double ray_tmin, double ray_tmax,
    ...
    if (height != infinity) {
        if ((axe == 'x' || axe == 'X') &&
            ...
            return false;
                   ...
                   (rec.p._y < center._y - height / 2 || rec.p._y > center._y + height / 2)) {
            return false;
        } else if ((axe == 'z' || axe == 'Z') &&
                   (rec.p._z < center._z - height / 2 || rec.p._z > center._z + height / 2)) {
            return false;
        }
    }
