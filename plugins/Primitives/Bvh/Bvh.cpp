...
RayTracer::Bvh::Bvh(Scene list)
    : Bvh(list._primitives, 0, list._primitives.size())
{
}

RayTracer::Bvh::Bvh(std::vector<std::shared_ptr<IPrimitives>> &objects,
    size_t start,
    size_t end)
{
  ...
  _bbox = std::make_shared<RayTracer::Aabb>(_left->bounding_box(),
                        _right->bounding_box());
}

~Bvh();

bool RayTracer::Bvh::hits(const RayTracer::Ray &r,
                          Math::Interval ray_t,
                          Material::Material &rec) const
{
  ...
  bool hit_right = _right.get()->hits(
      r, Math::Interval(ray_t._min, hit_left ? rec.t : ray_t._max), rec);
